...
const mock = vi.hoisted(() => ({
  auth: vi.fn(), customer: vi.fn(), cancel: vi.fn(), change: vi.fn(), upgrade: vi.fn(), reset: vi.fn(), usage: vi.fn(), entitlements: vi.fn(), listAddons: vi.fn(), purchase: vi.fn(), updateAddon: vi.fn(), cancelAddon: vi.fn(), testMode: vi.fn(),
  db: { billingProfile: { upsert: vi.fn() }, checkoutQuote: { create: vi.fn() }, site: { count: vi.fn() }, scan: { count: vi.fn() }, teamMember: { count: vi.fn() }, addOn: { findUnique: vi.fn() } },
...
vi.mock('@/lib/billing/mollie-flows', () => ({ createOrGetMollieCustomer: mock.customer, cancelSubscription: mock.cancel, changePlan: mock.change, createUpgradePayment: mock.upgrade, createPaymentMethodResetPayment: mock.reset }))
...
describe('checkout API', () => {
  ...
  it('omits unknown locale hints and tolerates optional persistence failures', async () => { mock.db.checkoutQuote.create.mockRejectedValue(new Error('db down')); mock.mollie.payments.update.mockRejectedValue(new Error('unsupported update')); expect((await createPayment(req({ plan: 'PRO', billingCountry: 'ZZ' }))).status).toBe(200); expect(mock.mollie.payments.create.mock.calls[0][0]).not.toHaveProperty('locale') })
  ...
  it.each([['audit', createAudit, { productId: Object.values(AUDIT_PRICES)[0].productId }], ['addon', createAddon, { type: 'EXTRA_SEAT' }]] as const)('handles %s redirect failure and missing checkout link', async (_, handler, body) => { mock.mollie.payments.update.mockRejectedValue(new Error('unsupported')); expect((await handler(req(body))).status).toBe(200); mock.mollie.payments.create.mockResolvedValue({ id: 'tr1', getCheckoutUrl: () => null }); expect((await handler(req(body))).status).toBe(500) })
})
