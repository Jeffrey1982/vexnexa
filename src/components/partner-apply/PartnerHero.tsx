    ...
    <section className="relative overflow-hidden border-b border-border/50 bg-gradient-to-b from-primary/[0.07] via-background to-background py-14 md:py-20 dark:from-primary/10">
      ...
      <div className="container relative mx-auto max-w-4xl px-4 text-center">
        ...
        <h1 className="font-display text-3xl font-bold tracking-tight text-foreground md:text-4xl lg:text-5xl">
          Join the VexNexa Pilot Partner Program
        </h1>
        <p className="mx-auto mt-5 max-w-2xl text-base leading-relaxed text-muted-foreground md:text-lg">
          Only <span className="font-semibold text-foreground">{spotsLeft}</span> spots remaining • Get
          Business Plan access + white-label reports + direct roadmap influence
        </p>
        <div className="mt-6 flex flex-col items-center gap-2">
          <ScarcityCounter spots={spotsLeft} />
        </div>
        <div className="mt-8 flex flex-col items-stretch gap-3 sm:mx-auto sm:max-w-md sm:items-center">
          <Button
            type="button"
            size="lg"
            className="gradient-primary h-12 text-base font-semibold shadow-lg shadow-primary/20"
            onClick={scrollToForm}
          >
            Apply Now – Limited to {spotsLeft} {spotsLeft === 1 ? "Partner" : "Partners"}
          </Button>
        </div>
        <PartnerStandardsBar
          className="mt-10 border-t-0 pt-2"
          introOverride="Powered by the same standards trusted by leading accessibility teams worldwide"
