...
export function renderReportHTML(data: ReportData): string {
  ...
  return `<!DOCTYPE html>
<html lang="en" data-style="${s}" data-report-version="v3">
<head>
<meta charset="UTF-8">
...
<body class="style-${s}">
<div class="running-header" aria-hidden="true"><span class="rh-left">${esc(data.domain)}</span><span class="rh-right">${fmtDate(data.scanDate)}</span></div>
<div class="running-footer" aria-hidden="true"><span class="rf-left">Generated by ${esc(brandName)}</span><span class="rf-center">Report v1.0 · ${esc(credit)}</span><span class="rf-right">Page <span class="page-num"></span></span></div>
...
/** Footer / running line: respect white-label “hide VexNexa” */
function reportFooterCredit(d: ReportData): string {
  return d.whiteLabelConfig.showVexNexaBranding
    ? "Branded by VexNexa · Powered by axe-core"
    : "Powered by axe-core";
}
...
function renderBrandBlock(d: ReportData, primary: string, isDark: boolean): string {
  ...
  const nameEl = hasName
    ? `<h3 class="brand-name" style="color:${nameColor}">${esc(displayName)}</h3>`
    : "";
...
function renderCover(d: ReportData, primary: string, s: ReportStyle): string {
  ...
  <header class="report-header report-header-light">
    ...
    <div class="header-meta">
      <span class="header-date">${fmtDate(d.scanDate)}</span>
      <span class="header-pill" style="color:${esc(ink)};border-color:${esc(primary)}">Accessibility compliance</span>
    </div>
  </header>
  <div class="cover-main cover-main-light">
    <div class="cover-main-left">
      <p class="cover-eyebrow">Automated Accessibility Audit</p>
      <h1 class="cover-title-light" style="color:${esc(ink)}">Accessibility compliance <span class="cover-title-soft">report</span></h1>
      <div class="cover-domain-block">
        <span class="cover-domain-label">Scanned Domain</span>
      ...
      <div class="cover-meta-grid cover-meta-light">
        <div class="cover-meta-cell"><span class="cover-meta-label">Standard</span><span class="cover-meta-value" style="color:${esc(primary)}">${esc(d.complianceLevel)}</span></div>
        <div class="cover-meta-cell"><span class="cover-meta-label">Risk</span><span class="cover-meta-value" style="color:${riskClr(d.riskLevel)}">${d.riskLevel}</span></div>
        <div class="cover-meta-cell"><span class="cover-meta-label">Scan date</span><span class="cover-meta-value">${fmtDate(d.scanDate)}</span></div>
        <div class="cover-meta-cell"><span class="cover-meta-label">Pages</span><span class="cover-meta-value">${pagesAnalyzed}</span></div>
      </div>
      ...
      <div class="cover-sev-pills">
        <span class="csp csp-crit"><strong>${d.issueBreakdown.critical}</strong> Critical</span>
        <span class="csp csp-ser"><strong>${d.issueBreakdown.serious}</strong> Serious</span>
        <span class="csp csp-mod"><strong>${d.issueBreakdown.moderate}</strong> Moderate</span>
        <span class="csp csp-min"><strong>${d.issueBreakdown.minor}</strong> Minor</span>
      </div>
    </div>
    <div class="cover-main-right">
      <div class="cover-score-panel">
        ${scoreRingSVG(hs.value, primary, BRAND_NAVY)}
        <p class="csc-label-light">${hs.label}</p>
  ...
  <footer class="cover-footer cover-footer-light">
    <span class="cover-scanid-light">Scan ${esc(d.scanId).slice(0, 8)}</span>
...
function renderExecutiveSummary(d: ReportData, primary: string, s: ReportStyle): string {
  ...
  <header class="exec-page-head">
    <p class="exec-eyebrow">Report overview</p>
    <h2 class="exec-page-title" style="color:${esc(ink)}">Executive summary</h2>
  ...
  <div class="exec-hero-panel">
    <div class="exec-hero-scorecol">
      <div class="exec-mega-score" style="color:${hsColor}">${hs.value}</div>
      <div class="exec-mega-sub"><span class="exec-mega-of">/100</span> <span class="exec-grade-pill" style="border-color:${hsColor};color:${esc(ink)}">Grade ${hs.grade}</span></div>
      <p class="exec-mega-label">${hs.label}</p>
      <div class="exec-mega-bar"><div class="exec-mega-bar-fill" style="width:${Math.min(100, hs.value)}%;background:linear-gradient(90deg,${esc(primary)} 0%,${BRAND_NAVY} 100%)"></div></div>
    </div>
    <div class="exec-hero-copy">
      <p class="exec-lead">The <strong>Accessibility Health Score</strong> reflects the number and severity of detected issues. This is the same score shown on your dashboard. A higher score indicates fewer and less severe accessibility barriers. Note: a high health score does not guarantee full WCAG compliance — critical violations can exist even at moderate scores.</p>
      <ul class="exec-lead-bullets">
        ...
        <li>Estimated effort: <strong>${esc(d.estimatedFixTime)}</strong></li>
      </ul>
    </div>
  </div>
  <div class="exec-cards exec-cards-premium">
    <div class="exec-card exec-card-premium">
      <h3>Site status</h3>
      <p><strong>${esc(d.domain)}</strong> — health score <strong style="color:${hsColor}">${hs.value}/100</strong> (grade ${hs.grade}).
    ...
    <div class="exec-card exec-card-premium">
      <h3>Remediation</h3>
      <p>Prioritise findings below by severity and user impact. Indicative effort for this snapshot: <strong style="color:${esc(primary)}">${esc(d.estimatedFixTime)}</strong>.</p>
    </div>
  </div>
  ${renderTopPriorityFixes(d.topPriorityFixes, s)}
  <h3 class="subsection-title metrics-heading-premium">Key metrics</h3>
...
/* Header row */
.report-header{display:flex;align-items:center;justify-content:space-between;padding:22px var(--space-xl) var(--space-md);flex-shrink:0}
.report-header-dark .brand-name{color:white}
.header-meta{display:flex;flex-direction:column;align-items:flex-end;gap:var(--space-xs)}
...
@media print{
  ...
  *{-webkit-print-color-adjust:exact;print-color-adjust:exact}
  ...
  .running-header{display:flex;position:fixed;top:0;left:0;right:0;justify-content:space-between;padding:8mm 16mm 0;font-size:9px;color:#9CA3AF;font-weight:500;letter-spacing:0.3px;z-index:100}
  .running-footer{display:flex;position:fixed;bottom:0;left:0;right:0;justify-content:space-between;padding:0 16mm 8mm;font-size:8px;color:#B0B5BD;font-weight:400;letter-spacing:0.2px;z-index:100}
