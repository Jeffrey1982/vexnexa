...
function scoreRingSVG(score: number, gradientStart: string, gradientEnd: string): string {
  ...
  const gc = score >= 80 ? "#8B4513" : score >= 60 ? "#b45309" : "#b91c1c";
...
function renderExecutiveSummary(d: ReportData, primary: string, s: ReportStyle): string {
  ...
  const metrics: { label: string; value: string | number; color: string }[] = [
    ...
    { label: d.labels.minor, value: d.issueBreakdown.minor, color: "#8B4513" },
...
<section class="page"${pi === 0 ? ' id="wcag-matrix"' : ''}>
  ...
  ${pi === 0 ? `<div class="matrix-legend">
    ...
    <span class="ml-item"><span class="wcag-status-chip" style="background:#FFF7ED;color:#EA580C">${esc(d.labels.needsManualReview)}</span> ${d.labels.locale === "nl" ? "Niet volledig automatisch te verifieren" : d.labels.locale === "fr" ? "Ne peut pas etre verifie entierement automatiquement" : "Cannot be fully verified automatically"}</span>
...
:root{
  ...
  --vn-navy:#1e3a8a;--vn-mint:#A0522D;--vn-surface:#F8FAFC;
